...
class Prepare(View):
    ...
    def post(self, request):
        ...
        return render(request, 'des_ui.html', context)

def func(request,args,txt):
    mode = args.get("mode")
    if mode == "ECB":
        des_ecb = ECB(args, txt)                        # 实例化ECB对象
        ...
        des_ecb.text = hex(int(f'0b{"".join(des_ecb.text)}',2))[2:].upper()

        return des_ecb.text

    elif mode == "CBC":
        des_cbc = CBC(args, txt)                        # 实例化ECB对象
        des_cbc.text = des_cbc.to_str_bin(des_cbc.text) # 将text转化为2进制矩阵
        des_cbc.iv = des_cbc.to_str_bin(des_cbc.iv)     # 将iv转化为2进制矩阵
        ...
        des_cbc.text = hex(int(f'0b{"".join(des_cbc.text)}', 2))[2:].upper()

        return des_cbc.text

    elif mode == "CTR":
        des_ctr = CTR(args, txt)
        des_ctr.text = des_ctr.to_str_bin(des_ctr.text)  # 将text转化为2进制矩阵
        des_ctr.iv = des_ctr.iv[:len(des_ctr.iv)-1] + hex( int(des_ctr.iv[-1],16) + 1 )[-1]   # 实现IV+1的操作(基于十六进制的+1)
        ...
        des_ctr.text = hex(int(f'0b{"".join(des_ctr.text)}', 2))[2:].upper()

        return des_ctr.text


    elif mode == "OFB":
        des_ofb = OFB(args, txt)
        ...
        des_cfb.text = hex(int(f'0b{"".join(des_cfb.text)}', 2))[2:].upper()
